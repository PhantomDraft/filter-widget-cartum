    ...
    uls.forEach((ul) => {
      ul.querySelectorAll("li").forEach((li) => {
        ...
        const name = titleEl
          ...
          : a.textContent.trim();
        const rawHref =
          a.getAttribute("data-fake-href") || a.getAttribute("href");
...
class FilterRenderer {
  ...
  constructor(
    ...
    this.insertMode = insertMode === "append" ? "append" : "replace";
    this.afterSelector = afterSelector;
    this.markup = Object.assign(
      {
        listClass: "",
        itemTag: "li",
        itemClass: "frontBrands-i",
        linkTag: "a",
        linkClass: "frontBrands-a filter-block",
        imgClass: "frontBrands-img filter-block__img",
        labelTag: "span",
        labelClass: "filter-block__label",
      },
      markup || {},
    );
    this.container = document.querySelector(targetSelector);
    console.log(
      "[FilterRenderer] init target=",
      targetSelector,
      "imageMap=",
      Object.keys(imageMap),
    );
    if (!this.container)
      console.warn(`[FilterRenderer] target "${targetSelector}" not found`);
  }

  render() {
    console.log(
      "[FilterRenderer] render() start, options=",
      this.options.length,
    );
    ...
    if (this.insertMode === "replace") {
      ...
      refNode = this.container.nextSibling;
      this.container.remove();
    } else {
      parent = this.container;
      if (this.afterSelector) {
        const after = parent.querySelector(this.afterSelector);
        refNode = after ? after.nextSibling : null;
      }
    }
    ...
    if (this.title) {
      ...
      parent.insertBefore(h, refNode);
    }

    // Helper to create a collapsed UL
    const baseClass =
      this.markup.listClass ||
      (this.container ? this.container.className : "frontBrands-list");
    const createList = () => {
    ...
    if (this.brandLast) {
      ...
      this.options.forEach((opt) => {
        const li = document.createElement(this.markup.itemTag);
        if (this.markup.itemClass) li.className = this.markup.itemClass;
        const a = document.createElement(this.markup.linkTag);
        a.href = opt.url;
        a.rel = "nofollow";
        if (this.markup.linkClass) a.className = this.markup.linkClass;
        a.title = opt.name;
        if (this.imageMap[opt.name]) {
          const img = document.createElement("img");
          img.src = this.imageMap[opt.name];
          img.alt = opt.name;
          if (this.markup.imgClass) img.className = this.markup.imgClass;
          a.appendChild(img);
        } else {
          const span = document.createElement(this.markup.labelTag);
          ...
          if (this.markup.labelClass) span.className = this.markup.labelClass;
          a.appendChild(span);
        }
        li.appendChild(a);
        (/\/filter\/brand=/.test(opt.url) ? brandUl : generalUl).appendChild(
          li,
        );
      });
    ...
    // Append lists
    lists.forEach((ul) => parent.insertBefore(ul, refNode));
    ...
    if (!this.autoExpand) {
      ...
      btn.href = "#";
      btn.textContent = this.expanderText;
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        lists.forEach((ul) =>
          ul.classList.replace("__collapsed", "__expanded"),
        );
        expander.remove();
      });
      expander.appendChild(btn);
      parent.insertBefore(expander, refNode);
    } else {
  ...
  init() {
    ...
    if (this.config.catalogUrl) {
      fetch(this.config.catalogUrl, { credentials: "same-origin" })
        ...
        .then((html) =>
          this.#processDoc(new DOMParser().parseFromString(html, "text/html")),
        )
  ...
  #processDoc(doc) {
    ...
    groups.forEach((g) => {
      ...
      const renderer = new FilterRenderer(
        ...
        g.insertMode ?? cfg.insertMode,
        g.afterSelector ?? cfg.afterSelector,
        Object.assign({}, cfg.markup, g.markup || {}),
      );
      renderer.render();
    });
  }
